...
| A60 | A small VPS or a local/office machine is the right training deployment category | Medium | Operational. [[Deployment Target Evaluation]] compared five categories against [[Training Deployment Architecture]] and recommends category 1 (small VPS) or category 4 (local/office machine), whichever MuleSoo already has the operational capacity to run — both fit the persistent-host requirement directly, both keep incremental cost near zero, and both avoid the "ephemeral unless configured correctly" risk in managed-container and PaaS categories. No vendor selected, no account created, nothing purchased or deployed. | [[Deployment Target Evaluation]] | **OPEN — a category is recommended, not a decision. See [[Decision Log]], proposed, not accepted** |
