...
| R44 | **§17 complaint handling has no mechanism** | 4 | 4 | 16 | **Opened by D144, and the highest open operational risk.** `CLAUDE.md` §17 obliges Telga to answer a *"paid but no airtime"* complaint by searching for the transaction and returning its state, with a final answer inside 24 hours. The founder has decided that Telga staff see **aggregates only**, so that search cannot be performed from the operations console, and nothing else performs it. **The obligation still stands; the means does not exist.** **What is still visible** and partially mitigates: per-shop pending and under-review counts on `/activity`, so staff can see *that* a shop has unresolved sales. What is not: which sale, for how much, or to whom — so a merchant naming a transaction cannot be answered. **Candidate resolutions, none yet chosen:** (a) individual rows inside an open support case, scoped to that shop — the option offered and not taken, and the most likely fit; (b) the merchant self-serves from their own device, where §17's data already is, and Telga answers from what the merchant reads out; (c) a narrow single-transaction lookup by exact id only, with a recorded reason and no browsing. **This must be resolved before a pilot**, because a complaint with no answer path is a merchant Telga cannot support | 
