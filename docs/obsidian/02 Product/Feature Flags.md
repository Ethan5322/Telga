...
| `deposit.chapa` | **on**, sandbox-only by construction | A signed Chapa agreement · NBE authorisation confirmed (§21) · legal review **before `money.live`** | **Chapa is an Ethiopian payment company**, used here as one *method* of a shop paying money in — beside carrying a slip to a bank counter (§20.1). The shop pays Chapa, Chapa tells Telga, Telga credits that shop’s selling balance. **Not a wallet, and not `payments.acceptance`**: that flag is Telga taking a *customer’s* money on a shop’s behalf and stays off. This is a shop paying in its own float, which is what §20 has always described. **The flag is not what keeps this safe.** A flag is one edit away from being wrong, so the guard sits where an edit cannot reach it: the adapter **refuses any key that is not `CHASECK_TEST-`**, and Telga refuses to start if the flag is on with a live key. There is no configuration of this build that reaches live Chapa. **And nothing credits by itself** — a verified payment becomes a `MATCHED` funding submission waiting for a person, exactly like a high-value bank deposit, on the founder’s instruction that *"admin panel approve it"*. Chapa’s own documentation demands the same shape: *"before giving value to a customer based on a webhook notification, always re-query our API"* — which is §20.1’s *"an SMS alert is a hint, not evidence"* arrived at independently by the people who run the payment network. Deliberately absent from `MOVES_REAL_MONEY`, for the same reason as `transfer.shop_to_shop`: what sits behind it moves none. See [[Decision Log]] D156 |
