...
| `pay.retry` | Try again |
| `bank_deposit.button` | Deposit money |
| `bank_deposit.title` | Deposit money at the bank |
| `bank_deposit.lede` | Print a slip, pay it into Telga’s bank account, and your selling balance goes up once Telga sees the payment. |
| `bank_deposit.amount_label` | How much are you paying in? |
| `bank_deposit.limits` | Between |
| `bank_deposit.print_slip` | Print payment slip |
| `bank_deposit.not_yet_money` | Printing a slip does not add balance. Telga adds it after the bank confirms your payment. |
| `bank_deposit.slip_title` | Bank payment slip |
| `bank_deposit.slip_status` | Take this slip to the bank. Nothing has been added to your balance yet. |
| `bank_deposit.bank` | Bank |
| `bank_deposit.account_name` | Account name |
| `bank_deposit.account_number` | Account number |
| `bank_deposit.reference` | Payment reference |
| `bank_deposit.expires` | Pay before |
| `bank_deposit.quote_reference` | Write this reference on the deposit form. Without it Telga cannot tell whose money it is. |
| `bank_deposit.never_personal_account` | Pay only to this account. Never to an employee’s personal account. |
| `bank_deposit.credited_later` | Your balance is added only after Telga confirms this payment with the bank. |
| `bank_deposit.already_open` | You already have a slip waiting to be paid. Pay that one, or cancel it before printing another. |
| `bank_deposit.view_slip` | Show the slip again |
| `bank_deposit.cancel` | Cancel this slip |
| `bank_deposit.cancel_confirm` | Cancel this slip? If you have already paid it, do not cancel — contact Telga. |
| `bank_deposit.no_account` | Telga has no bank account configured, so no slip can be printed. Contact Telga. |
| `bank_deposit.refused.shop_not_active` | This shop cannot take deposits at the moment. Contact Telga. |
| `bank_deposit.refused.amount_invalid` | Enter a whole number of birr. |
| `bank_deposit.refused.amount_below_minimum` | That is below the smallest deposit Telga accepts. |
| `bank_deposit.refused.amount_above_maximum` | That is above the largest single deposit. Pay in more than once. |
| `bank_deposit.refused.order_already_open` | You already have a slip waiting to be paid. |
| `bank_deposit.refused.deposits_disabled` | Bank deposits are switched off. |
| `bank_deposit.close` | Close |
| `bank_deposit.close_note` | Closing this screen does not cancel the slip. Your reference stays valid until it expires. |
| `bank_deposit.test_account` | TEST SLIP. These bank details are not real and no bank will accept this. For practice only. |
| `pay.deposit.entry` | Deposit with Telga Pay |
| `transfer.title` | Send balance to another shop |
| `transfer.lede` | Move balance you already have to another Telga shop. This is not a deposit — no new money comes in. |
| `transfer.recipient` | Receiving device ID |
| `transfer.device_hint` | Ask the other shop for their device ID. It is shown on their Settings screen. Never ask for their device key. |
| `transfer.pin` | Your PIN |
| `transfer.send` | Send balance |
| `transfer.confirm` | Send this balance? Once it is sent it cannot be taken back. |
| `transfer.fee` | Fee |
| `transfer.sent` | Balance sent. |
| `transfer.final` | A sent transfer cannot be reversed unless both shops agree and Telga approves it. |
| `transfer.needs_approval` | Telga must approve this transfer before it is sent. |
| `transfer.not_moved` | Nothing has moved yet. Do not tell the other shop to expect it. |
| `transfer.training_only` | Training only. No real money moves and no bank is involved. |
| `transfer.refused.sender_not_active` | This shop cannot send balance at the moment. |
| `transfer.refused.recipient_unknown` | No shop uses that device ID. Check it with them. |
| `transfer.refused.recipient_not_active` | That shop cannot receive balance at the moment. |
| `transfer.refused.same_shop` | That is this shop’s own device. |
| `transfer.refused.insufficient` | You do not have that much available. |
| `transfer.refused.over_limit` | That is more than this shop may send in one day. |
| `transfer.refused.amount_invalid` | Enter a whole number of birr. |
| `chapa.button` | Deposit with Chapa |
| `chapa.title` | Deposit with Chapa |
| `chapa.lede` | Pay into Telga through Chapa. Your balance goes up on its own once Telga confirms the payment with Chapa. |
| `chapa.pay_now` | Open Chapa to pay |
| `chapa.slip_title` | Chapa payment slip |
| `chapa.slip_status` | Pay this on Chapa. Nothing has been added to your balance yet. |
| `chapa.method` | How to pay |
| `chapa.method_value` | Chapa — card, mobile money or bank |
| `chapa.credited_later` | Telga adds your balance automatically once Chapa confirms the payment. You do not need to tell anyone. |
| `chapa.keep_reference` | Keep this reference. If anything goes wrong, it is what Telga needs to find your payment. |
| `chapa.refused.unavailable` | Chapa could not start this payment. Nothing was charged. Try again, or use a bank deposit. |
| `chapa.refused.disabled` | Chapa deposits are switched off. |
